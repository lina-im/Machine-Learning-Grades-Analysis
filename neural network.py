...
mlp = MLPClassifier(hidden_layer_sizes=(100,), max_iter=100, early_stopping=True, random_state=42)
mlp.fit(X_train, y_train)

#predict and evaluate
y_pred = mlp.predict(X_test)
overall_accuracy = accuracy_score(y_test, y_pred)
print(f"Overall Accuracy (Neural Network): {overall_accuracy:.4f}")
...
plt.title('Neural Network - Prediction Accuracy by Gender')
plt.ylabel('Accuracy')
plt.ylim(0, 1)
plt.grid(axis='y')
plt.tight_layout()
